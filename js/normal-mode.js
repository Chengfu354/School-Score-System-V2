...
function renderNormalModePage() {
    ...
    activeIds.forEach(id => {
        ...
        if (sub) {
            theadHtml += `<th style="min-width: 65px; position: sticky; top: 0; background: var(--card-bg); z-index: 2; text-align: center; font-size: 13.5px; padding: 10px 8px; white-space: nowrap;">${escapeHtml(sub.name)}</th>`;
        }
    });
    theadHtml += `
        <th style="min-width: 80px; position: sticky; top: 0; background: var(--card-bg); z-index: 2; text-align: center; color: var(--primary-color); font-size: 13.5px; padding: 10px 8px; white-space: nowrap;">ពិន្ទុសរុប</th>
        <th style="min-width: 80px; position: sticky; top: 0; background: var(--card-bg); z-index: 2; text-align: center; color: var(--primary-color); font-size: 13.5px; padding: 10px 8px; white-space: nowrap;">មធ្យមភាគ</th>
        <th id="th-sort-rank" style="min-width: 80px; position: sticky; top: 0; background: var(--card-bg); z-index: 4; text-align: center; cursor: pointer; user-select: none; color: var(--primary-color); font-size: 13.5px; padding: 10px 8px; line-height: 1.2; white-space: nowrap;">
            ...
            tbodyHtml += `
                <td style="padding: 6px 4px; text-align: center;">
                    <input type="number" 
                           ...
                           ${readonlyAttr}
                           style="width: 100%; text-align: center; border: 1px solid transparent; background: transparent; font-weight: 600; font-size: 13.5px; ${!isNormalEditMode ? 'cursor: default;' : ''}">
                </td>
            `;
        });

        // 4 Read-Only Result Columns (Ensure center alignment matching request)
        tbodyHtml += `
            <td id="total-${s.id}" style="background: var(--bg-color); font-weight: 700; color: var(--primary-color); text-align: center; font-size: 13.5px; padding: 10px 8px; white-space: nowrap;"><b>${displayTotal}</b></td>
            <td id="avg-${s.id}" style="background: var(--bg-color); font-weight: 600; text-align: center; font-size: 13.5px; padding: 10px 8px; white-space: nowrap;">${displayAvg}</td>
            <td id="rank-${s.id}" style="background: var(--bg-color); text-align: center; vertical-align: middle; padding: 10px 8px; white-space: nowrap;">
                <span class="${rankClass}" style="margin: 0 auto; display: inline-flex; align-items: center; justify-content: center;">${s.rank || '-'}</span>
            </td>
            <td id="grade-${s.id}" style="background: var(--bg-color); text-align: center; padding: 10px 8px; white-space: nowrap;"><span class="badge ${gradeClass}">${s.grade || '-'}</span></td>
        </tr>`;
    });
